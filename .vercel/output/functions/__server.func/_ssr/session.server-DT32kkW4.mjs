...
//#region node_modules/.nitro/vite/services/ssr/assets/session.server-DT32kkW4.js
...
async function isListedAdminEmail(companyId, email) {
	const value = (email ?? "").trim().toLowerCase();
	if (!value) return false;
	return (await listAdminEmails(companyId)).includes(value);
}
...
async function bootstrapProfile(opts) {
	const sql = await getSql();
	const existing = await sql.query(`${EMP_SELECT} where e.user_id = $1`, [opts.userId]);
	if (existing[0]) return finishProfile(existing[0], opts);
	const email = (opts.email ?? "").trim().toLowerCase();
	if (email) {
		const byEmail = await sql.query(`${EMP_SELECT} where lower(e.email) = $1 and e.user_id is null limit 1`, [email]);
		if (byEmail[0]) {
			await sql`
        update employees
	...
	if (!listed && !settings.signupOpen) throw Object.assign(/* @__PURE__ */ new Error("New sign-in is turned off. Ask an administrator to approve your account."), { status: 403 });
	const names = (opts.name ?? "New Technician").trim().split(/\s+/);
	const id = newId("emp");
	const number = `E-${Math.floor(300 + Math.random() * 600)}`;
	const role = listed ? "admin" : "technician";
  ...
  `;
    ...
    `;
	return finishProfile((await sql.query(`${EMP_SELECT} where e.id = $1`, [id]))[0], opts);
}
async function finishProfile(row, opts) {
	const sql = await getSql();
	let emp = mapEmployee(row);
	if (await isListedAdminEmail(emp.companyId, opts.email || emp.email) && (emp.role !== "admin" || emp.accountStatus !== "active")) {
		await sql`
      update employees
      set role = 'admin',
          account_status = 'active',
...
async function requireProfile(userId) {
	const rows = await (await getSql()).query(`${EMP_SELECT} where e.user_id = $1`, [userId]);
	if (!rows[0]) throw Object.assign(/* @__PURE__ */ new Error("No employee profile"), { status: 403 });
...
//#endregion
export { requireProfile as A, listAdminEmails as C, navForRole as D, mapEmployee as E, verifyAdminCode as M, writeAudit as N, newId as O, writeSetting as P, initials as S, loadSettings as T, formatHours as _, THEMES as a, getSql as b, assertActive as c, bootstrapProfile as d, cn as f, formatDuration as g, formatClock as h, NAV_CATALOG as i, todayIso as j, num as k, assertAdmin as l, ensureDbReady as m, DEFAULT_ROLE_NAV as n, addAdminEmail as o, dockForRole as p, EMP_SELECT as r, adminHintVisible as s, DEFAULT_ADMIN_CODE as t, assertManager as u, formatMoney as v, listEmployees as w, hashAdminCode as x, getPglite as y };
