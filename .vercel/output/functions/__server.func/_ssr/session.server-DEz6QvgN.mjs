...
//#region node_modules/.nitro/vite/services/ssr/assets/session.server-DEz6QvgN.js
...
async function bootstrapProfile(opts) {
	...
	const unlinkedAdmin = await sql.query(`${EMP_SELECT} where e.role = 'admin' and e.user_id is null order by e.created_at asc limit 1`);
	if (unlinkedAdmin[0]) {
...
async function finishProfile(row, opts) {
	...
	if (await isListedAdminEmail(emp.companyId, opts.email || emp.email) && emp.role !== "admin") {
		await sql`
      update employees
      set role = 'admin',
...
async function listEmployees(companyId, activeOnly = false) {
	const sql = await getSql();
	return (activeOnly ? await sql.query(`${EMP_SELECT} where e.company_id = $1 and e.active = true order by e.last_name, e.first_name`, [companyId]) : await sql.query(`${EMP_SELECT} where e.company_id = $1 order by e.role, e.last_name`, [companyId])).map(mapEmployee);
}
...
//#endregion
export { requireProfile as C, writeSetting as D, writeAudit as E, num as S, verifyAdminCode as T, listAdminEmails as _, assertAdmin as a, mapEmployee as b, cn as c, formatDuration as d, formatHours as f, hashAdminCode as g, getSql as h, adminHintVisible as i, ensureDbReady as l, getPglite as m, EMP_SELECT as n, assertManager as o, formatMoney as p, addAdminEmail as r, bootstrapProfile as s, DEFAULT_ADMIN_CODE as t, formatClock as u, listEmployees as v, todayIso as w, newId as x, loadSettings as y };
