...
import { D as getSql, L as newId, R as num, V as requireProfile, Y as writeAudit, m as assertManager } from "./session.server-BThkfVCN.mjs";
import { t as createServerRpc } from "./createServerRpc-CcvdN_gc.mjs";
import { t as shopMiddleware } from "./shop-middleware-By_PdWYR.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/api-parts-9oXX3bl2.js
...
var searchParts = createServerFn({ method: "GET" }).validator((d) => d ?? {}).middleware([shopMiddleware]).handler(searchParts_createServerFn_handler, async ({ context, data }) => {
...
var getTruck = createServerFn({ method: "GET" }).validator((d) => d ?? {}).middleware([shopMiddleware]).handler(getTruck_createServerFn_handler, async ({ context, data }) => {
	const profile = await requireProfile(context.userId);
...
var useTruckPart = createServerFn({ method: "POST" }).validator((d) => d).middleware([shopMiddleware]).handler(useTruckPart_createServerFn_handler, async ({ context, data }) => {
	const profile = await requireProfile(context.userId);
	const sql = await getSql();
	const row = (await sql`
...
var replenishTruck = createServerFn({ method: "POST" }).validator((d) => d).middleware([shopMiddleware]).handler(replenishTruck_createServerFn_handler, async ({ context, data }) => {
	const profile = await requireProfile(context.userId);
