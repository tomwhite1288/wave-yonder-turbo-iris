...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/health", "/api/health"):
            return self._json({"ok": True, "dir": DATA})
        if path == "/api/sync":
        ...
        with LOCK:
            ...
            if action == "backup":
                ...
                nxt = {
                    ...
                    "at": now_ms(),
                    "data": merge_data(cur.get("data"), body.get("data")),
                    "actor": (auth["user"]["user"] if auth else "backup"),
                }
                blob_put("sync-" + key, nxt)
                backup_dir = os.path.join(DATA, "backups")
                os.makedirs(backup_dir, exist_ok=True)
                path = os.path.join(backup_dir, "%s-%s.json" % (safe_key(key), time.strftime("%Y%m%d-%H%M%S")))
                write_json(path, nxt)
                return self._json({"ok": True, "rev": nxt["rev"], "file": path})
